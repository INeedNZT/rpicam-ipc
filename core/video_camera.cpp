...
void VideoCamera::ConfigureVideo()
{
...
void VideoCamera::StartEncoder()
{
	...
	encoder_->SetInputDoneCallback([this](void *mem)
								   { 
		// If non-NULL, mem would indicate which mem(buffer) has been completed, but
...
void VideoCamera::SetEncodeOutputReadyCallback(EncodeOutputReadyCallback callback)
{
	encoder_->SetOutputReadyCallback(callback);
}
